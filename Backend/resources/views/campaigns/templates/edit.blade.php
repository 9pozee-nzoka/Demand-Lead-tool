...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <!-- Header -->
    <div class="mb-8">
        <h1 class="text-3xl font-semibold text-neutral-900 dark:text-white">Edit Template</h1>
        <p class="mt-2 text-sm text-neutral-500 dark:text-neutral-400">Update email template: {{ $template->name }}</p>
    </div>

    <form action="{{ route('campaigns.templates.update', $template) }}" method="POST">
        @csrf
        @method('PATCH')
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <textarea name="description" 
                                  id="description" 
                                  rows="2"
                                  class="w-full px-4 py-2 bg-white dark:bg-neutral-900 border border-neutral-300 dark:border-neutral-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent text-neutral-900 dark:text-white">{{ old('description', $template->description) }}</textarea>
                        @error('description')
                            <p class="mt-1 text-sm text-rose-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <select name="category" 
                                ...
                                <option value="nurture" {{ old('category', $template->category) === 'nurture' ? 'selected' : '' }}>Nurture</option>
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <textarea name="body" 
                                  ...
                                  rows="15"
                                  required
                                  class="w-full px-4 py-2 bg-white dark:bg-neutral-900 border border-neutral-300 dark:border-neutral-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent text-neutral-900 dark:text-white font-mono text-sm">{{ old('body', $template->body) }}</textarea>
                        @error('body')
                            <p class="mt-1 text-sm text-rose-600">{{ $message }}</p>
                        @enderror
                        <p class="mt-1 text-xs text-neutral-500 dark:text-neutral-400">
